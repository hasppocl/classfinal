package com.hz.classfinal.plugin;


import com.hz.classfinal.Constants;
import com.hz.classfinal.JarEncryptor;
...
public class ClassFinalPlugin extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        try {
            String path = encryptor.doEncryptJar();
            long t2 = System.currentTimeMillis();
            logger.info("Encrypt " + encryptor.getEncryptFileCount() + " classes");
            logger.info("Encrypted " + project.getPackaging() + " [" + path + "]");
            logger.info("Encrypt complete");
            logger.info("Time [" + ((t2 - t1) / 1000d) + " s]");
            logger.info("");
        } catch (Exception e) {
            e.printStackTrace();
            throw new MojoExecutionException("Encrypt jar error", e);
        }
    }
